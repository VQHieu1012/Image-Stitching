...
int mainVideo(int blend_mode, int resolution_mode, const char* src1, const char* src2)
{
    ...
    {   
        ...
        try{
            ...
            img_1_ptr = desc_1;
            // 442
            surfDetDes(img_0_ptr, ipts_0, 0, true, 5, 4, 2, 0.002f);        
            surfDetDes(img_1_ptr, ipts_1, 0, true, 5, 4, 2, 0.002f);        
            ...
            // cout << "fps_count: "<< fps_count<<endl;

            drawFPS(display);

            cvShowImage("stitched", display);
            // imwrite("./output/frame_0.jpg", stitched);
...
int main(int argc, char* argv[]) 
{
    ...
    switch (mode)
    {
        ...
        case 1 : // run static image match between a pair of images
            if (src1 == "") src1 = "./image/1.jpg"; // if not provided, use sample image
            if (src2 == "") src2 = "./image/2.jpg"; // if not provided, use sample image
            return mainStitch(blend_mode, src1.c_str(), src2.c_str());

                
        case 2 : // run image stitching with local video files
            if (src1 == "") src1 = "./video/1.mp4"; // if not provided, use sample video
            if (src2 == "") src2 = "./video/2.mp4"; // if not provided, use sample video
